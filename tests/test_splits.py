...
class SplitGenerationTests(unittest.TestCase):
    def setUp(self):
        # Three balanced classes with two reserved validation examples and four
        # post-validation training examples per class.
    ...
    def test_max_k_uses_smallest_class_when_pool_is_unbalanced(self):
        unbalanced_pool = self.train_pool_indices[:-1]
        self.assertEqual(max_k_per_class(self.targets, unbalanced_pool), 3)
